...
class CeleryClient:
    ...
    def execute(self, command, parameter):
        ...
        def revoke(*args):
            ...
            if 'ok' in response[0][destination] and 'tasks unknown' not in response[0][destination]['ok']:
                response = {'status': 'success',
                            'message': 'Task %s on server %s has been revoked.' % (task_id, destination)}
            else:
                response = {'status': 'failure',
                            'message': 'Task %s on server %s does not exist.' % (task_id, destination)}
            return response
        ...
        def terminate(*args):
            ...
            if 'ok' in response[0][destination] and 'tasks unknown' not in response[0][destination]['ok']:
                response = {'status': 'success',
                            'message': 'Task %s on server %s has been terminated.' % (task_id, destination)}
            else:
                response = {'status': 'failure',
                            'message': 'Task %s on server %s does not exist.' % (task_id, destination)}
            return response
        ...
        def restart(*args):
            ...
            if response and 'ok' in response[0][destination]:
                ...
                response = {'status': 'failure', 'message': 'Worker on %s reload failure. Reason:%s' % (
                destination, response[0][destination]['error'])}
            else:
                response = {'status': 'failure', 'message': 'Parameter or errors happen, Please check.'}
            return response
        ...
        def poolgrow(*args):
            ...
            response = ctrl.pool_grow(n=grown_num, reply=True,
                                      destination=[destination])
            if len(response) != 0 and response[0][destination].has_key('ok') and 'pool will grow' in \
                    response[0][destination]['ok']:
                response = {'status': 'success',
                            'message': 'Pool on server %s will grow %d pool.' % (destination, grown_num)}
            elif len(response) != 0 and response[0][destination].has_key('error'):
                response = {'status': 'failure',
                            'message': 'Parameter or errors happen, Please check,reason:%s' % response[0][destination][
                                'error']}
            else:
                response = {'status': 'failure', 'message': 'Parameter or errors happen, Please check.'}
            return response
        ...
        def poolshrink(*args):
            ...
            response = ctrl.pool_shrink(n=shrink_num, reply=True,
                                        destination=[destination])
            if len(response) != 0 and response[0][destination].has_key(
                    'error') and 'Can\'t shrink pool. All processes busy!' in response[0][destination]['error']:
                response = {'status': 'failure',
                            'message': 'Can\'t shrink pool. All processes busy on server %s! Please try this later.' % (
                                destination)}
            elif len(response) != 0 and response[0][destination].has_key('ok') and 'pool will shrink' in \
                    response[0][destination]['ok']:
                response = {'status': 'success',
                            'message': 'pool on server %s will shrink %d pool.' % (destination, shrink_num)}
            elif len(response) != 0 and response[0][destination].has_key('error'):
                response = {'status': 'failure', 'message': 'Parameter or errors happen, Please check. Reason:%s' % (
                response[0][destination]['error'])}
            else:
                response = {'status': 'failure', 'message': 'Parameter or errors happen, Please check.'}
            return response
